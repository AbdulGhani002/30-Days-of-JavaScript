...
document.addEventListener("keydown", (e) => {
  ...
  if (e.repeat) return;
  const noteDetail = getNoteDeatil(keyboardKey);
...
function getNoteDeatil(keyboardKey) {
...
function playNotes() {
  NOTE_DETAILS.forEach((n) => {
    ...
    if (n.noteDetil.oscillator != null) {
      n.oscillator.stop();
...
function startNote(noteDetil, gain) {
  const gainNode = audioContext.createGain();
  gainNode.gain.value = gain;
  const oscillator = audioContext.createOscillator();
  oscillator.frequency = noteDetil.frequency;
  oscillator.type = "sine";
  oscillator.frequency.value = noteDetil.frequency;
  oscillator.connect(gainNode).connect(audioContext.destination);
  oscillator.start();
  noteDetil.oscillator = oscillator;
}
*/
const audioContext = new AudioContext();
